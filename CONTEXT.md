...
**Assistant Catalog**:
The durable, app-owned set of Assistant Books for every audiobook Library the current Audiobookshelf User Identity has cached. It contains only that identity's projection and is kept in step with cached catalogs and retained downloads so Assistant Actions can search it without the app's UI or scripting runtime being awake.
...
- **EPUB Read-Along** moves the reader to a new **Resource** when the Listening Position crosses into one, and scrolls within a Resource to keep the current **Text Unit** in view while **Follow Mode** holds.
- **EPUB Read-Along** seeks the Listening Position to a **Text Unit**'s start when the user taps that unit's text, and resumes **Follow Mode** when it does.
- **EPUB Read-Along** does nothing when the user taps text no **Text Unit** covers, rather than seeking to the nearest one.
...
- An **Assistant Book** keeps the same identity when its **Server Connection Endpoint** changes.
- The **Assistant Catalog** belongs to the current **Audiobookshelf User Identity** and stores every audiobook **Library** that identity has cached, not only the **Active Library**.
- Assistant search, books-by-author, play-by-name, **Suggested Assistant Books**, and system-search indexing answer only from the **Active Library** slice of the **Assistant Catalog**; with no **Active Library** they ask the user to open the app and choose one. Opening or resuming a specific **Assistant Book** by identity, and Resume, are not narrowed to the Active Library.
- A search **Assistant Action** that offers app results opens the app's Search surface for the **Active Library** with the spoken text prefilled; it never opens a separate results surface, and a request bound to another identity or Library is discarded.
...
- A retained audiobook **Downloaded Audio Asset** remains an **Assistant Book** when its server catalog row becomes missing, using its retained presentation metadata.
...
- A **Session Entry Switch** physically replaces the **Assistant Catalog** with the new **Audiobookshelf User Identity**'s books; rows for the previous identity are not retained in the Assistant Catalog.
...
- **Suggested Assistant Books** are drawn from the **Active Library** slice of the **Assistant Catalog**, ordered in-progress first, then downloaded, then **Favorite**, and never exceed 25.
...
- Read-only **Assistant Actions** (is-in-library, books-by-author) answer from the **Active Library** slice of the **Assistant Catalog** for a chosen **User Session**, including **Session Needs Sign-In** and an **Offline User Session**.
- In **Signed-Out Required Sign-In**, every **Assistant Action** replies that sign-in is required and offers to open the app.
- In **Downloaded-Only Mode** after explicit logout, assistant surfaces remain disabled until **User Session Entry** chooses an identity again.
- A play **Assistant Action** may start a **Downloaded Audio Asset** for the chosen **User Session**, and may stream only with a signed-in or **Offline User Session**.
...
- **Assistant Books** are indexed for system search from the **Active Library** slice of the **Assistant Catalog**; choosing one opens that audiobook as the **Current Audiobook** and does not start playback.
...
- Control Center and Action-button **Assistant Actions** are deferred beyond the first Assistant Actions release.
